...
Everything indented under the for loop is run for each element. The `:` is the signal to Python that everything underneath the for loop

The indenting (usually via 2 spaces) is a signal that the code goes together, much like curly brackets in R:
...
Most editors (including colab) will automatically indent using spaces when you type a `:`
...
To end the code block, add a line after it
...
## Just for Fun: iterating over a string

Before you run this, what do you think the for loop does?

```{pyodide}
my_string = "my cat likes cat food"
...
- Iterating over rows in an efficent way will require us to write *functions*, so stay tuned!
